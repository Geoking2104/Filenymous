/**
 * Composant racine — v2 + fluid CSS animations
 */
...
const CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .net-pill.is-online span {
    animation: pulseGlow 2s ease-in-out infinite;
  }
  .progress-fill {
    transition: width .4s var(--ease);
    background-size: 200% 100%;
    background-image: linear-gradient(90deg, #22d3ee, #2dd4bf, #22d3ee);
    animation: shimmer 2s linear infinite;
  }
  .top-tabs button {
    transition: color .2s ease, background .2s ease, border-color .2s ease, transform .15s var(--ease), box-shadow .2s ease;
  }
  ...
  .btn-primary, .btn-ghost, .btn-success, .btn-danger {
    transition: transform .15s var(--ease), box-shadow .2s ease, background .2s ease, border-color .2s ease, opacity .2s ease;
  }
  ...
  button { font-family:inherit; font-size:inherit; cursor:pointer; border:none; border-radius:var(--radius); padding:.6rem 1.2rem; }
  ...
  .brand-mark { width:38px; height:38px; border-radius:10px; display:grid; place-items:center; background:transparent; }
  ...
  .net-pill { display:inline-flex; align-items:center; gap:.4rem; padding:.35rem .7rem; border:1px solid var(--border); border-radius:999px; color:var(--muted); font-size:.75rem; background:rgba(255,255,255,.05); white-space:nowrap; transition: border-color .2s ease, color .2s ease; }
  ...
  .peer-card { min-height:118px; display:grid; place-items:center; gap:.28rem; padding:.9rem; transition: transform .2s var(--ease), border-color .2s ease; }
  ...
  .room-drop { min-height:170px; border:2px dashed rgba(255,255,255,.2); border-radius:18px; display:grid; place-items:center; text-align:center; gap:.25rem; padding:1.2rem; cursor:pointer; background:rgba(255,255,255,.03); color:var(--text); transition: border-color .2s ease, background .2s ease, transform .2s var(--ease); }
  .room-drop:hover { border-color: rgba(34,211,238,.4); background: rgba(34,211,238,.04); }
  ...
  .room-drop.is-dragging { background:rgba(34,211,238,.08); border-color:var(--g1); transform: scale(1.01); }
  ...
  .room-transfer-row { display:flex; justify-content:space-between; gap:.8rem; padding:.55rem .7rem; border:1px solid var(--border); border-radius:12px; background:rgba(255,255,255,.05); transition: background .2s ease; }
  .room-transfer-row:hover { background: rgba(255,255,255,.08); }
  ...
  .room-messages p { display:grid; gap:.1rem; padding:.65rem .75rem; border:1px solid var(--border); border-radius:12px; background:rgba(255,255,255,.05); animation: fadeInUp .3s var(--ease) both; }
  ...
  if (isLinkDl) {
    return (
      <>
        ...
        <div className="app">
          <Header minimal />
          <main className="main panel-enter"><ReceivePanel /></main>
          <Footer />
        </div>
      </>
    );
